 ...
 * una factory per Flattener, che a sua volta modella un trasformatore da liste
 * di liste a
 * liste, che funziona ricombinando le inner list in input producendo (alla
 * fine, o mano a mano)
 * elementi in uscita.
 * 
 ...
 * l'esercizio, ma concorrono
 * comunque al raggiungimento della totalità del punteggio:
 * - implementazione del quinto metodo della factory (ossia, a scelta se ne
 * realizzino 4, il quinto è opzionale)
 * - concisione del codice e rimozione di tutte le ripetizioni con eventuale uso
 * appropriato di pattern
 *
...
public class Test {
	...
	public void testSumEach() {
		...
		// ogni inner list produce in uscita la somma dei suoi elementi, ossia 1+2+3+4,
		// 0, 20, 10+10+10, 0
	...
	public void testConcatPairs() {
		...
		// le inner list vengono prese a coppie: per ogni coppia si uniscono tutte le
		// loro stringhe
		// se c'è una inner list finale la si tratta da sola
		assertEquals(List.of("abc", "cdef", "gh"),
				f.flatten(List.of(List.of("a", "b"), List.of("c"), List.of("c", "d", "e"), List.of("f"),
						List.of("g", "h"))));
	...
	public void testEach() {
		...
		// ogni inner list produce la concatenazione dei suoi elementi -- che sono
		// stringhe
		assertEquals(List.of("ab", "c", "cde", "f", "g"),
				f.flatten(
						List.of(List.of("a", "b"), List.of("c"), List.of("c", "d", "e"), List.of("f"), List.of("g"))));
	...
	public void testSumVectors() {
		...
		// l'output è prodotto a fine iterazione: le inner list sono considerate come
		// vettori (nel senso dell'algebra) da sommare
		assertEquals(List.of(1111, 2222, 3333),
				f.flatten(List.of(List.of(1, 2, 3), List.of(10, 20, 30), List.of(100, 200, 300),
